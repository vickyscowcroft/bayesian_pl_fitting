...
def load_data(P_ref=0.52854, FeH_ref=-1.4):
    ...
    ## updated_rrl.txt is spitzer data
    #tbl = np.genfromtxt('updated_rrl.txt', names='name, type, blazhko, period, log10fP, FeH, prior_mu, prior_mu_err, SF_ebv, SF_ebv_err, m_U_obs, m_U_obs_err, m_B_obs, m_B_obs_err, m_hipp_obs, m_hipp_obs_err, m_V_obs, m_V_obs_err, m_R_obs, m_R_obs_err, m_I_obs, m_I_obs_err, m_z_obs, m_z_obs_err, m_J_obs, m_J_obs_err, m_H_obs, m_H_obs_err, m_K_obs, m_K_obs_err,  m_I1_obs, m_I1_obs_err, m_I2_obs, m_I2_obs_err, m_W3_obs, m_W3_obs_err, post_mu, post_mu_err',delimiter=",", dtype='|U10, |U10, bool, f, f, f, f, f, f, f, f, f, f, f, f, f, f, f, f, f, f, f, f, f, f, f, f, f, f, f, f, f, f, f, f, f, f, f', skip_header=1)
    ...
    # add TGAS parallaxes
    tgas = np.genfromtxt('dr2_parallaxes.txt', names=True)
    ...
    for b, ext_coeff in enumerate(ext_coeffs):
        #print('m_%s_obs' % bands[b].decode('utf-8'))
        obs_mags[:, b] = tbl['m_%s_obs' % bands[b].decode('utf-8')]
        obs_magErrs[:, b] = tbl['m_%s_obs_err' % bands[b].decode('utf-8')]
    ...
    # I1, I2, W3: Eqs. 2-7 of Klein et al. (2014)
    ...
    # use broad priors for the I1 and I2 bands
...
def lnprob(theta, dat, bands_to_process=[10], rel_tolerance=1e-09, FeH_ref=-1.4, P_ref=0.52854):
    ...
    for b, band in enumerate(bands_to_process):
        ...
        for star in np.where(np.isfinite(dat['obs_mags'][:, band]))[0]:
            ...
            Mpred = betas[b] + alphas[b]*dat['log10fP'][star] + gammas[b]*(dat['FeH'][star] - FeH_ref)
            # variance in the predicted absolute magnitude
            var_Mpred = (gammas[b]*dat['FeHErr'][star])**2 + (alphas[b]*0.02*dat['log10fP'][star])**2
...
def run_emcee(dat, bands_to_process=[10], rel_tolerance=1e-09, debug=True):
    ...
    good = np.abs(dat['sigma_par']/dat['par_obs']) < 1000
    #good = dat['FeH'] > -2.0
    #good = dat['type'] == 'RRab'
    ...
    dat_good = dat.copy()
    dat_good['log10fP'] = dat_good['log10fP'][good]
    ...
    if debug:
        ...
        #for param in np.arange(ndim):
            #acors.append(autocorr.integrated_time(np.mean(sampler.chain, axis=0)[:, param])[0])
